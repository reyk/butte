...
impl PrimitiveLiteralToken for ast::FloatingConstant {
    ...
    fn fmt_lit(&self, ty: impl Spanned + fmt::Display) -> Self::TokenType {
        let float_ty = format!("{}", ty);
        match float_ty.as_ref() {
            "f32" => Self::TokenType::new(&format!("{}{}", *self as f32, float_ty), ty.span()),
            _ => Self::TokenType::new(&format!("{}{}", self, float_ty), ty.span()),
        }
    }
}
